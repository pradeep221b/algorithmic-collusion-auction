...
\end{tikzpicture}
\caption{The mechanism behind the memory-0 price. Rungs are the bid grid. On the left, each step down the grid needs a completed chain of learning events, and each link needs an exploratory draw at the right moment. On the right, a firm at the bottom raises its bid in one event because it has nothing to lose. The asymmetry concentrates the exploring process near the top; freezing the tables samples that distribution.}
\label{fig:mechanism}
\end{figure}
