...
``Pre-registered'' in this paper means written down in the project documents before the corresponding runs; there is no public registry entry. The collapse criterion for the ablation was fixed before the ablation was run. The punishment test follows the procedure of Section~\ref{sec:indicators}; its two 12-round windows and half-rung threshold were fixed before any verdict was read and never changed.
% TODO-FACT: confirm the last sentence; if the windows and threshold were written down before the first punishment run, say so instead.
The windows are 12 rounds long so that a converged cycle of period 1, 2, 3, 4 or 6 completes a whole number of times in each; cycles of period 5 or 7, which occur in 9 of the 100 long-run seeds, are not covered and can be misread (Section~\ref{sec:discussion}), and at 1M rounds, where periods run up to 56 and only 23 of 100 seeds have period four or less, the 12-seed verdicts of Table~\ref{tab:m3} are more exposed to this misreading than the long-run ones. The Phase~1 experiments were designed after the results of Section~\ref{sec:results} were seen; their five predictions were written down before the corresponding runs and are listed with their outcomes, two of which failed, in Section~\ref{sec:phase1}.
...
Every configuration is run on 100 independent seeds; where a table reports a subset (the seeds with $\Dl > 0.5$, labelled ``collusive'' in Section~\ref{sec:results}) it says so. A seed fixes the random draws of a run; the initial tables are deterministic. Means are reported with a 95\% confidence interval of mean $\pm 1.96$ times the standard error across seeds; medians, ranges and per-seed counts are reported with the price levels in Section~\ref{sec:results}. A 50-seed run of the whole study preceded the 100-seed run; every $\Dl$ moved by at most 0.02 between the two.
% TODO-FACT: whether the 50 seeds are the first 50 of the 100.

\paragraph{Two horizons.}
At the specified $T = 1$M rounds the learners are almost greedy ($\varepsilon_t = 4.5 \times 10^{-5}$, Section~\ref{sec:learners}), yet $\Dl = 0.375$, no seed meets the convergence criterion and M2 is not met (Section~\ref{sec:results}). The 5M-round horizon is not in the specification: it was added after that result was seen and is not pre-registered, and every base configuration converges in every seed there.
% TODO-FACT: whether the 30M-round $\varepsilon = 10^{-4}$ occupation point was among the pre-registered levels; if not, name it here as a second post-hoc addition.
We report both horizons: M2 fails at 1M and is met at 5M, M4 and M5 give the same verdict at both, and M3 is judged on 12 seeds at 1M and on all 100 at 5M; the headline $\Dl = 0.621$ comes from the added one. Every run goes to its full horizon.
