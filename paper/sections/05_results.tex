...
At the specified horizon the full learner reaches $\Dl = 0.375$ (CI 0.354--0.397), a mean frozen clearing price of \eur{43.8}/MWh, and no seed has converged. The spread is wide: 1 seed below 0.2, 85 between 0.2 and 0.5, and 14 at or above 0.5. Twelve of those 14 are strictly above; the other two sit exactly at $\Dl = 0.5$, a frozen mean price of \eur{55.00}, which is why the table reports 12\%.
Figure~\ref{fig:trajectory} shows the process still moving. Averaged over seeds the price starts at \eur{54.9}, first falls to \eur{40} after about 120,000 rounds while $\varepsilon_t$ is still large, reaches its minimum of \eur{37.5} at about 196,000 rounds, and then drifts up to \eur{43.9} by the horizon; a few seeds jump to a higher band before it. We read the early fall as the learners finding the undercut, but this section does not test that reading.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/price_trajectory.png}
\caption{Mean clearing price per block of 1,000 training rounds, full learner, 1M rounds. Thin lines are the 100 seeds; the thick line is their mean. Dashed lines mark the competitive (\eur{10}) and collusive (\eur{100}) benchmarks.}
\label{fig:trajectory}
\end{figure}

Five million rounds finish the process (Figure~\ref{fig:trajectory5m}). Seeds leave the \eur{40} band one at a time, jump to a mean price between \eur{60} and \eur{72}, and never move again. Every seed converges; the convergence round recorded for each run is the first round after which no firm's greedy action changed for 100,000 rounds, and it has median 1.63M (range 1.20M--2.88M).
All land in a narrow band: $\Dl = 0.621$ (CI 0.612--0.630), 99 seeds in $[0.5, 0.7)$ and one above, mean frozen price \eur{65.9}/MWh. The acceptance criterion for M2 fails at the pre-registered 1M horizon and is met at 5M, a horizon added after the 1M result was seen and not itself pre-registered (Section~\ref{sec:design}). The specification asks for ``a meaningful fraction of seeds converge to $\Dl > 0.5$'' and fixes no number; we read 12\% of seeds above $\Dl = 0.5$ with none converged at 1M as failing it, and 100\% with all converged at 5M as meeting it, and a reader who counts 12\% as meaningful should treat M2 as met at both horizons.
...
The $\gamma = 0$ learner converges to long, irregular cycles (20 of 100 seeds with period four or less, periods up to 25) whose most common last-round profiles have one firm at cost: $(10,16,16)$ in 13 seeds, $(10,42,42)$ in 10 and $(10,36,36)$ in 9. Because these seeds sit on long cycles, a last-round profile is one snapshot of a cycle and these counts shift by about three seeds per profile with the round sampled; the shape, one firm at cost and two tied above it, does not.
...
\caption{M3 in the 5M full-learner runs. Left: bids and clearing price around the forced deviation for seed 19, the seed with the highest $\Dl$ (the figure's agents 0, 1, 2 are firms 1, 2, 3). Right: mean clearing price across all 100 seeds, with the no-deviation counterfactual dotted.}
